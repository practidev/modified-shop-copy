<?php
/* --------------------------------------------------------------
   $Id: categories_view.php 15445 2023-08-23 13:58:28Z GTB $
                 ...
                 ?>
                   ...
                   <td class="categories_view_data txta-c">
                     ...
                     if ($categories['categories_status'] == '1') {
                       echo xtc_image(DIR_WS_IMAGES . 'icon_status_green.gif', IMAGE_ICON_STATUS_GREEN, 12, 12) . '<a href="' . xtc_href_link(FILENAME_CATEGORIES, xtc_get_all_get_params(array('cPath', 'action', 'pID', 'cID')) . 'action=setcflag&flag=0&cID=' . $categories['categories_id'] . '&cPath=' . $cPath) . '">&nbsp;&nbsp;' . xtc_image(DIR_WS_IMAGES . 'icon_status_red_light.gif', IMAGE_ICON_STATUS_RED_LIGHT, 12, 12) . '</a>';
                     } else {
                       echo '<a href="' . xtc_href_link(FILENAME_CATEGORIES, xtc_get_all_get_params(array('cPath', 'action', 'pID', 'cID')) . 'action=setcflag&flag=1&cID=' . $categories['categories_id'] . '&cPath=' . $cPath) . '">' . xtc_image(DIR_WS_IMAGES . 'icon_status_green_light.gif', IMAGE_ICON_STATUS_GREEN_LIGHT, 12, 12) . '</a>&nbsp;&nbsp;' . xtc_image(DIR_WS_IMAGES . 'icon_status_red.gif', IMAGE_ICON_STATUS_RED, 12, 12);
                     }
                     ?>
                   </td>
                   <td class="categories_view_data txta-c">--</td>
               ...
               if (isset($pInfo) && (is_object($pInfo)) && ($products['products_id'] == $pInfo->products_id) ) {
                 ...
                 <td class="categories_view_data txta-c">
                   ...
                   if ($products['products_status'] == '1') {
                     echo xtc_image(DIR_WS_IMAGES . 'icon_status_green.gif', IMAGE_ICON_STATUS_GREEN, 12, 12) . '<a href="' . xtc_href_link(FILENAME_CATEGORIES, xtc_get_all_get_params(array('cPath', 'action', 'pID', 'cID')) . 'action=setpflag&flag=0&pID=' . $products['products_id'] . '&cPath=' . $cPath) . '">&nbsp;&nbsp;' . xtc_image(DIR_WS_IMAGES . 'icon_status_red_light.gif', IMAGE_ICON_STATUS_RED_LIGHT, 12, 12) . '</a>';
                   } else {
                     echo '<a href="' . xtc_href_link(FILENAME_CATEGORIES, xtc_get_all_get_params(array('cPath', 'action', 'pID', 'cID')) . 'action=setpflag&flag=1&pID=' . $products['products_id'] . '&cPath=' . $cPath) . '">' . xtc_image(DIR_WS_IMAGES . 'icon_status_green_light.gif', IMAGE_ICON_STATUS_GREEN_LIGHT, 12, 12) . '&nbsp;&nbsp;</a>' . xtc_image(DIR_WS_IMAGES . 'icon_status_red.gif', IMAGE_ICON_STATUS_RED, 12, 12);
                   }
                   ?>
                 </td>
                 <td class="categories_view_data txta-c">
                   ...
                   if ($products['products_startpage'] == '1') {
                     echo xtc_image(DIR_WS_IMAGES . 'icon_status_green.gif', IMAGE_ICON_STATUS_GREEN, 12, 12) . '<a href="' . xtc_href_link(FILENAME_CATEGORIES, xtc_get_all_get_params(array('cPath', 'action', 'pID', 'cID')) . 'action=setsflag&flag=0&pID=' . $products['products_id'] . '&cPath=' . $cPath) . '">&nbsp;&nbsp;' . xtc_image(DIR_WS_IMAGES . 'icon_status_red_light.gif', IMAGE_ICON_STATUS_RED_LIGHT, 12, 12) . '</a>';
                   } else {
                     echo '<a href="' . xtc_href_link(FILENAME_CATEGORIES, xtc_get_all_get_params(array('cPath', 'action', 'pID', 'cID')) . 'action=setsflag&flag=1&pID=' . $products['products_id'] . '&cPath=' . $cPath) . '">' . xtc_image(DIR_WS_IMAGES . 'icon_status_green_light.gif', IMAGE_ICON_STATUS_GREEN_LIGHT, 12, 12) . '&nbsp;&nbsp;</a>' . xtc_image(DIR_WS_IMAGES . 'icon_status_red.gif', IMAGE_ICON_STATUS_RED, 12, 12);
                   }
                   ?>
                 </td>
                 <td class="categories_view_data txta-c">
              ...
              if (isset($_POST['multi_move']) && xtc_not_null($_POST['multi_move'])) {
                ...
                if (isset($_POST['multi_categories']) && is_array($_POST['multi_categories'])) {
                  foreach ($_POST['multi_categories'] AS $multi_category) {
                    ...
                    $contents[] = array('text' => '<b>' . $cInfo->categories_name . '</b>');
                    if ($cInfo->childs_count > 0) {
                      $contents[] = array('text' => '<div class="messageStackError">'.sprintf(TEXT_MOVE_WARNING_CHILDS, $cInfo->childs_count).'</div>');
                    }
                    if ($cInfo->products_count > 0) {
                      $contents[] = array('text' => '<div class="messageStackError">'.sprintf(TEXT_MOVE_WARNING_PRODUCTS, $cInfo->products_count).'</div>');
                    }
                  }
                  $category_tree = xtc_get_category_tree();
                  $params = ((isset($cInfo) && is_object($cInfo)) ? '&cID=' . $cInfo->categories_id : '');
                }
                ...
                if (isset($_POST['multi_products']) && is_array($_POST['multi_products'])) {
                  if ($current_category_id != 0) {
                    foreach ($_POST['multi_products'] AS $multi_product) {
                      $contents[] = array('text' => '<b>' . xtc_get_products_name($multi_product) . '</b>');
                      $contents[] = array('text' => xtc_output_generated_category_path($multi_product, 'product'));
                      $contents[] = array('text' => '<hr noshade>');
                    }
                    $category_tree = xtc_get_category_tree('0','','0');
                  } else {
                    $contents[] = array('text' => '<div class="messageStackError">'.TEXT_NO_MOVE_POSSIBLE.'</div>');
                    $dispnone = ' dispnone';
                  }
                  $params = ((isset($pInfo) && is_object($pInfo)) ? '&pID=' . $pInfo->products_id : '');
                }
                
                if (isset($category_tree) && is_array($category_tree)) {
                  $contents[] = array('text' => '<strong>' . TEXT_MOVE_ALL . '</strong>');
                  $contents[] = array('text' => xtc_draw_pull_down_menu('move_to_category_id', $category_tree, $current_category_id));
                }
                ...
                $contents[] = array('align' => 'center', 'text' => '<input class="button'.$dispnone.'" type="submit" name="multi_move_confirm" value="' . BUTTON_MOVE . '"> <a class="button" href="' . xtc_href_link(FILENAME_CATEGORIES, xtc_get_all_get_params(array('cPath', 'action', 'pID', 'cID')) . (isset($_GET['cPath']) ? 'cPath=' . $cPath : '') . $params) . '">' . BUTTON_CANCEL . '</a>');
              }
              ...
              if (isset($_POST['multi_delete']) && xtc_not_null($_POST['multi_delete'])) {
                ...
                if (isset($_POST['multi_categories']) && is_array($_POST['multi_categories'])) {
                  foreach ($_POST['multi_categories'] AS $multi_category) {
                    ...
                    $contents[] = array('text' => '<b>' . $cInfo->categories_name . '</b>');
                    if ($cInfo->childs_count > 0) {
                      $contents[] = array('text' => '<div class="messageStackError">'.sprintf(TEXT_DELETE_WARNING_CHILDS, $cInfo->childs_count).'</div>');
                    }
                    if ($cInfo->products_count > 0) {
                      $contents[] = array('text' => '<div class="messageStackError">'.sprintf(TEXT_DELETE_WARNING_PRODUCTS, $cInfo->products_count).'</div>');
                    }
                  }
                  $params = ((isset($cInfo) && is_object($cInfo)) ? '&cID=' . $cInfo->categories_id : '');
                }
                ...
                $contents[] = array('align' => 'center', 'text' => '<input class="button" type="submit" name="multi_delete_confirm" value="' . BUTTON_DELETE . '"> <a class="button" href="' . xtc_href_link(FILENAME_CATEGORIES, xtc_get_all_get_params(array('cPath', 'action', 'pID', 'cID')) . (isset($_GET['cPath']) ? 'cPath=' . $cPath : '') . $params) . '">' . BUTTON_CANCEL . '</a>');
              }
              ...
              if (isset($_POST['multi_copy']) && xtc_not_null($_POST['multi_copy'])) {
                ...
                if (isset($_POST['multi_categories']) && is_array($_POST['multi_categories'])) {
                  foreach ($_POST['multi_categories'] AS $multi_category) {
                    ...
                    $contents[] = array('text' => '<b>' . $cInfo->categories_name . '</b>');
                    if ($cInfo->childs_count > 0) {
                      $contents[] = array('text' => sprintf(TEXT_MOVE_WARNING_CHILDS, $cInfo->childs_count));
                    }
                    if ($cInfo->products_count > 0) {
                      $contents[] = array('text' => sprintf(TEXT_MOVE_WARNING_PRODUCTS, $cInfo->products_count));
                    }
                  }
                  $params = ((isset($cInfo) && is_object($cInfo)) ? '&cID=' . $cInfo->categories_id : '');
                }
                ...
                if (QUICKLINK_ACTIVATED == 'true') {
                  ...
                  for ($i=0;$n=sizeof($cat_tree),$i<$n;$i++) {
                    $tree .= xtc_draw_checkbox_field('dest_cat_ids[]', $cat_tree[$i]['id']) . ' ' . $cat_tree[$i]['text'].'<br />';
                  }
                  $contents[] = array('text' => $tree.'<br /><hr noshade>');
                  $contents[] = array('text' => '<b>'.TEXT_SINGLECOPY.'</b><br />'.TEXT_SINGLECOPY_DESC.'<br /><br />');
                }
            ...
            default:
              if ($rows > 0) {
                if (isset($cInfo) && is_object($cInfo)) {
                  ...
                  $contents[] = array('align' => 'center', 'text' => '<input type="submit" class="button" name="multi_delete" value="'. BUTTON_DELETE . '">'.((!xtc_not_null($search)) ? '&nbsp;<input type="submit" class="button" name="multi_move" value="' . BUTTON_MOVE . '">' : '').'&nbsp;<input type="submit" class="button" name="multi_copy" value="' . BUTTON_COPY . '">');
                  ...
                  //Multi Element Actions
                  $contents[] = array('align' => 'center', 'text' => '<div style="padding-top: 5px; font-weight: bold; width: 100%;">' . TEXT_MARKED_ELEMENTS . '</div>');
                  $contents[] = array('align' => 'center', 'text' => xtc_button(BUTTON_DELETE, 'submit', 'name="multi_delete"').((!xtc_not_null($search) && $current_category_id != 0) ? '&nbsp;'.xtc_button(BUTTON_MOVE, 'submit', 'name="multi_move"') : '').'&nbsp;'.xtc_button(BUTTON_COPY, 'submit', 'name="multi_copy"'));
                  $contents[] = array('align' => 'center', 'text' => '<input type="submit" class="button" name="multi_status_on" value="'. BUTTON_STATUS_ON . '">&nbsp;<input type="submit" class="button" name="multi_status_off" value="' . BUTTON_STATUS_OFF . '">');
                  ...
                  $contents[] = array('align' => 'center', 
                                      'text' =>  '<a class="button" href="'.xtc_href_link(FILENAME_CATEGORIES, xtc_get_all_get_params(array('action', 'current_product_id', 'cPath', 'pID')).'action=edit_crossselling&last_action=list&current_product_id='.$pInfo->products_id.'&cpath='.$pInfo->cPath).'">' . BUTTON_EDIT_CROSS_SELLING . '</a>
                                                  <a class="button" href="' . xtc_href_link(FILENAME_REVIEWS, xtc_get_all_get_params(array('action', 'pID')).'pID='.$pInfo->products_id.'&cPath='.$pInfo->cPath) . '">' . BUTTON_REVIEWS . '</a>
