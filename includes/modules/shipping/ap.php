<?php
/* -----------------------------------------------------------------------------------------
   $Id: ap.php 15442 2023-08-23 06:30:59Z GTB $   
  ...
  class ap {
    ...
    function __construct() {
      ...
      $this->sort_order = ((defined('MODULE_SHIPPING_AP_SORT_ORDER')) ? MODULE_SHIPPING_AP_SORT_ORDER : '');
      $this->icon = DIR_WS_ICONS . 'shipping_ap.gif';
      $this->tax_class = defined('MODULE_SHIPPING_AP_TAX_CLASS') ? MODULE_SHIPPING_AP_TAX_CLASS : '';
      $this->enabled = ((defined('MODULE_SHIPPING_AP_STATUS') && MODULE_SHIPPING_AP_STATUS == 'True') ? true : false);
      $this->num_zones = defined('MODULE_SHIPPING_AP_NUMBER_ZONES') ? (int)MODULE_SHIPPING_AP_NUMBER_ZONES : 0;

      if ($this->enabled == true 
          && !defined('RUN_MODE_ADMIN')
          && (int)MODULE_SHIPPING_AP_ZONE > 0 
          && is_object($order)
          )
      {
        $check_flag = false;
        $check_query = xtc_db_query("SELECT zone_id 
                                      ...
                                      WHERE geo_zone_id = '" . (int)MODULE_SHIPPING_AP_ZONE . "' 
    ...
    function quote($method = '') {
      ...
      if ($dest_zone == 0) {
        ...
        for ($i=0, $n=count($ap_table); $i<$n; $i+=2) {
