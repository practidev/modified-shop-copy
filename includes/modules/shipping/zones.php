<?php
/* -----------------------------------------------------------------------------------------
   $Id: zones.php 15442 2023-08-23 06:30:59Z GTB $
  ...
  class zones {
    ...
    function __construct() {
      ...
      $this->num_zones = defined('MODULE_SHIPPING_ZONES_NUMBER_ZONES') ? (int)MODULE_SHIPPING_ZONES_NUMBER_ZONES : 0;

      if ($this->enabled == true 
          && !defined('RUN_MODE_ADMIN')
          && (int)MODULE_SHIPPING_ZONES_ZONE > 0 
          && is_object($order)
          )
      {
        $check_flag = false;
        $check_query = xtc_db_query("SELECT zone_id 
                                      ...
                                      WHERE geo_zone_id = '" . (int)MODULE_SHIPPING_ZONES_ZONE . "' 
